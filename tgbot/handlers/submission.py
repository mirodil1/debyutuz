...
from keyboards.reply import skip_keyboard, location_keyboard, education_level_keyboard
...
async def process_area(message: Message, state: FSMContext, repo: RequestsRepo):
    ...
    await state.set_state(SubmissionState.education_level)

    await message.answer(
        "<b>Ma'lumotingizni kiriting.</b>\n",
        reply_markup=education_level_keyboard()
    )


@submission_router.message(SubmissionState.education_level, F.text.in_(
        [
            "O'rta",
            "O'rta maxsus",
            "Tugallanmagan oliy",
...
async def process_education_level(message: Message, state: FSMContext):
    education_level_map = {
        "O'rta": "school",
        "O'rta maxsus": "college",
        "Tugallanmagan oliy": "bachelor",
        "Oliy": "masters"
    }
    await state.update_data(education_level=education_level_map[message.text])
    await state.set_state(SubmissionState.education)

    await message.answer(
        "<b>Tamomlagan ta'lim muassasasini kiriting.</b>\n"
        "<i>(Namuna: Toshkent Davlat Texnika Universiteti)</i>",
        reply_markup=ReplyKeyboardRemove()
    )


@submission_router.message(SubmissionState.education_level)
async def process_education_level(message: Message, state: FSMContext):
    await message.answer(
        "<b>Iltimos, ma'lumotingizni quyidagi tugmalardan tanlang.</b>",
        reply_markup=education_level_keyboard()
    )
    return
